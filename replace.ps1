# 获取用户输入的替换字符串
$newString = Read-Host "请输入要替换 'TrafficLawLearningSystem' 的新字符串"
...
foreach ($dir in $directories) {
    if ($dir.Name -like "*TrafficLawLearningSystem*") {
        $newName = $dir.Name -replace "TrafficLawLearningSystem", $newString
...
Get-ChildItem -Path . -File -Recurse | ForEach-Object {
    if ($_.Name -like "*TrafficLawLearningSystem*") {
        $newName = $_.Name -replace "TrafficLawLearningSystem", $newString
...
Get-ChildItem -Path . -File -Recurse | ForEach-Object {
    ...
    if ($content -like "*TrafficLawLearningSystem*") {
        $newContent = $content -replace "TrafficLawLearningSystem", $newString
